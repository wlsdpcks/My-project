...
namespace Valve.VR.InteractionSystem
{
	...
	{
		[Tooltip( "Virtual transform corresponding to the meatspace tracking origin. Devices are tracked relative to this." )] //  사이버 공간 의 가상 세계와 대비되는 물리적 세계 및 환경(meatspace) 추적 원점에 해당하는 가상 변환 장치는 이를 기준으로 추적됩니다.
        public Transform trackingOriginTransform;

		[Tooltip( "List of possible transforms for the head/HMD, including the no-SteamVR fallback camera." )] // SteamVR이 없는 폴백 카메라를 포함하여 헤드/HMD에 가능한 변형 목록입니다.
        public Transform[] hmdTransforms;

		[Tooltip( "List of possible Hands, including no-SteamVR fallback Hands." )] // SteamVR이 없는 대체 손을 포함하여 가능한 손 목록(배열)
        public Hand[] hands;

		[Tooltip( "Reference to the physics collider that follows the player's HMD position." )] // 플레이어의 HMD 위치를 따르는 물리 충돌기에 대한 참조
        public Collider headCollider;

		[Tooltip( "These objects are enabled when SteamVR is available" )] // 이러한 개체는 SteamVR을 사용할 수 있을 때 활성화됩니다.
        public GameObject rigSteamVR;

		[Tooltip( "These objects are enabled when SteamVR is not available, or when the user toggles out of VR" )] // 이러한 개체는 SteamVR을 사용할 수 없거나 사용자가 VR에서 전환할 때 활성화됩니다.
        public GameObject rig2DFallback;

		[Tooltip( "The audio listener for this player" )]
		public Transform audioListener;

        [Tooltip("This action lets you know when the player has placed the headset on their head")] // 이 작업을 통해 플레이어가 머리에 헤드셋을 착용한 시점을 알 수 있습니다.
        ...
        //-------------------------------------------------
        // Singleton instance of the Player. Only one can exist at a time. 플레이어의 싱글톤 인스턴스 한 번에 하나만 존재할 수 있습니다.
        //-------------------------------------------------
