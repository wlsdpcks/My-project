...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        {
            SnapOnAttach = 1 << 0, // The object should snap to the position of the specified attachment point on the hand. 개체는 손의 지정된 부착점 위치에 스냅되어야 합니다.
            DetachOthers = 1 << 1, // Other objects attached to this hand will be detached. 이 손에 부착된 다른 개체는 분리됩니다.
            DetachFromOtherHand = 1 << 2, // This object will be detached from the other hand. 이 개체는 다른 손에서 분리됩니다.
            ParentToHand = 1 << 3, // The object will be parented to the hand. 개체는 손의 부모가 됩니다.
            VelocityMovement = 1 << 4, // The object will attempt to move to match the position and rotation of the hand. 개체는 손의 위치와 회전에 맞게 이동하려고 시도합니다.
            TurnOnKinematic = 1 << 5, // The object will not respond to external physics. 개체는 외부 물리학에 반응하지 않습니다.
            TurnOffGravity = 1 << 6, // The object will not respond to external physics. 개체는 외부 물리학에 반응하지 않습니다.
            AllowSidegrade = 1 << 7, // The object is able to switch from a pinch grab to a grip grab. Decreases likelyhood of a good throw but also decreases likelyhood of accidental drop
                                     // 개체는 핀치 그랩에서 그립 그랩으로 전환할 수 있습니다. 잘 던질 가능성이 줄어들지만 실수로 떨어질 가능성도 줄어듭니다.
