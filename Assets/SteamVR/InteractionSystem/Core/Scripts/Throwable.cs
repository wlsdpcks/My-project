...
namespace Valve.VR.InteractionSystem
{
	...
	{
		[EnumFlags]
		[Tooltip( "The flags used to attach this object to the hand." )] //이 개체를 손에 부착하는 데 사용되는 플래그입니다.
        public Hand.AttachmentFlags attachmentFlags = Hand.AttachmentFlags.ParentToHand | Hand.AttachmentFlags.DetachFromOtherHand | Hand.AttachmentFlags.TurnOnKinematic;

        [Tooltip("The local point which acts as a positional and rotational offset to use while held")] // 잡고 있는 동안 사용할 위치 및 회전 오프셋 역할을 하는 로컬 포인트
        public Transform attachmentOffset;

		[Tooltip( "How fast must this object be moving to attach due to a trigger hold instead of a trigger press? (-1 to disable)" )] // 방아쇠를 누르는 대신 방아쇠를 잡고 있기 때문에 이 물체가 부착하려면 얼마나 빨리 움직여야 합니까? (비활성화하려면 -1)
        public float catchingSpeedThreshold = -1;

        public ReleaseStyle releaseVelocityStyle = ReleaseStyle.GetFromHand;

        [Tooltip("The time offset used when releasing the object with the RawFromHand option")] // RawFromHand 옵션으로 개체를 놓을 때 사용되는 시간 오프셋
        public float releaseVelocityTimeOffset = -0.011f;

        public float scaleReleaseVelocity = 1.1f;

        [Tooltip("The release velocity magnitude representing the end of the scale release velocity curve. (-1 to disable)")] // 스케일 릴리스 속도 곡선의 끝을 나타내는 릴리스 속도 크기입니다. (비활성화하려면 -1)
        ...
        // 이 곡선을 사용하여 측정된 방출 속도의 크기에 따라 스케일링된 방출 속도로 쉽게 이동합니다. 이렇게 하면 떨어뜨리기, 던지기, 던지기를 더 잘 구분할 수 있습니다
        public AnimationCurve scaleReleaseVelocityCurve = AnimationCurve.EaseInOut(0.0f, 0.1f, 1.0f, 1.0f);

        [Tooltip( "When detaching the object, should it return to its original parent?" )] // 개체를 분리할 때 원래 부모로 돌아가야 합니까?
